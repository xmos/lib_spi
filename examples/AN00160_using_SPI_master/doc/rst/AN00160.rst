...
Looking at this in a more detail you can see the following:
  ...
  - The ``spi_master`` tasks drives the SPI bus and takes the ports it
  ...
  - The ``app`` task communicates to the ``spi_master`` task via the
    ...
    SPI master tasks could connect to many other tasks in parallel.
...
by the first argument. In this cases it is ``0``, so the zero-th
...
binary in the ``bin`` folder of the project.
...
By clicking on the :menuitem:`Run` icon (a green arrow), the program
...
By dragging in the four ports used by the application you can see that
the application has driven the correct SPI signal.
