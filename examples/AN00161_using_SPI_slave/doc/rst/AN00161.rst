...
simulation) to the the SPI slave. This allows use to generate traffic
to show the communication functioning.
...
Looking at this in a more detail you can see the following:
  ...
  - The ``spi_slave`` tasks controls the application
    SPI bus and takes the ports it will use as arguments.

  - The ``reg_file`` task is connected to the ``app`` tasks and the
    ``spi_slave`` task.
  ...
  - The ``spi_master`` tasks controls the test SPI bus and takes
